...
import { DEPLOYED_AT, DEPLOYER, SYMBOL, explorerAddress } from '../deployment';
import type { Panel } from './types';

export function createFooter(): Panel {
  const root = el('footer', { class: 'site-footer reveal' },
    el('p', {}, `Read-only view of a testnet deployment — USDG is Paxos's testnet Global Dollar and the collateral is Robinhood's ${SYMBOL} token; the price feed and the IRM are mocks (the testnet has no Chainlink feeds). `,
