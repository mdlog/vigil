...
contract VigilPremiumTest is Base {
    ...
    function test_accrualEqualsBorrowedTimesMultTimesDeltaIndex() public {
        ...
        assertApproxEqRel(e.paid, expectedDue, 0.001e18); // 5 %/yr interest adds a little debt over the weekend
        assertEq(usdg.balanceOf(address(backstop)) - bsBefore, e.paid - e.paid / 100); // 99 % to the backstop
        assertEq(so.bountyPool(address(nvda)), e.paid / 100); // 1 % poke bounty
    ...
    function test_accrualIndependentOfCallFrequency() public {
        ...
        // only the notional sampling (5 %/yr interest) differs, not the time integral (INV-7)
    ...
    function test_membershipAndDelinquency() public {
        _open(mB, alice, 10e18, 0.86e18); // max LTV: m(b) = 1, premium ≈ 3.5 bp/weekend on ~1,032 USDG
        _open(mB, bob, 10e18, 0.5e18);
        assertFalse(premium.isMember(idB, bob)); // never opted in
        assertTrue(premium.isDelinquent(idB, bob));
        _join(alice, 1e6); // 1 USDG ≥ the 7-day reserve (≈ 0.92 USDG)
        assertTrue(premium.isMember(idB, alice));
        assertFalse(premium.isDelinquent(idB, alice));
        assertGt(premium.minReserve(idB, alice), 0);
        // cannot withdraw below the reserve
        vm.prank(alice);
        vm.expectRevert(VigilPremium.ReserveBreach.selector);
        premium.withdrawUnused(idB, 100e6);
        // an escrow drained after 3 weekends → delinquent → not a member; topUp restores it
    ...
    /// Two Vigil markets (e.g. NVDA 86 % and 77 %) on one VigilPremium — the approve to SESSION happens only once.
